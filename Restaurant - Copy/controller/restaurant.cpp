...
Meal *Restaurant::filterMeal(int id)
{
    for (vector<Meal *>::iterator i = mealList.begin(); i != mealList.end(); i++)
    {
        if ((*i)->getId() == id)
            return *i;
    }
...
int Restaurant::createDish(int id, string name, vector<string> ingridient, vector<double> needQuantity, double price)
{
    ...
    existed = this->filterDish(id) ? true : this->filterDish(name) ? true : false;
    if (existed)
                           ...
                           vector<int> dishQuant)
{
    ...
    vector<Meal *>::iterator it = this->mealList.end();
    if (it == this->mealList.end())
        id = 0;
    else
        id = (*(it - 1))->getId() + 1;
    meal_ptr = new Meal(table_ptr, list, date, start, end, waiter_ptr, client_ptr, id);
    return this->SUCCESS;
}
...
int Restaurant::deleteMeal(int id)
{
    cout << "haha";
    Meal *ptr = this->filterMeal(id);
    if (!ptr)
        return this->NON_EXISTING_OBJECT;
    for (vector<Meal *>::iterator it = this->mealList.begin(); it != this->mealList.end(); it++)
    {
        if ((*it)->getId() == id)
        {
            cout << "haha";
            this->mealList.erase(it);
        }
    }
    return SUCCESS;
}

int Restaurant::deleteTable(int id)
{
    Table *ptr = this->filterTable(id);
    if (!ptr)
        return this->NON_EXISTING_OBJECT;
    for (vector<Table *>::iterator it = this->tableList.begin(); it != this->tableList.end(); it++)
    {
        if ((*it) == ptr)
        {
